<script lang="ts">
  ...
  interface Props { price: VariantPrice; }
  ...
  function buildPts(p: VariantPrice): { labels: string[]; data: number[] } {
    const now = p.market; const t = p.trends;
  ...
  const currentPriceLinePlugin = {
    ...
    afterDraw(chart: any) {
      ...
      ctx.save();
      ctx.setLineDash([4, 4]); ctx.strokeStyle = color; ctx.lineWidth = 1; ctx.globalAlpha = 0.6;
      ctx.beginPath(); ctx.moveTo(chartArea.left, y); ctx.lineTo(chartArea.right, y); ctx.stroke();
      const label = `Current: ${pricingStore.formatPrice(currentPrice, price.currency)}`;
      ctx.setLineDash([]); ctx.globalAlpha = 0.7; ctx.font = '10px Geist, sans-serif'; ctx.fillStyle = color; ctx.textAlign = 'right';
  ...
  async function initChart() {
    ...
    const { labels, data } = buildPts(price); const dir = getDirection(price); const lineColor = getLineColor(dir);
    ...
    chartInstance = new ChartJS.Chart(chartCanvas, {
      type: 'line',
      data: { labels, datasets: [{ data, borderColor: lineColor, borderWidth: 2, pointRadius: 3, pointHoverRadius: 5, pointBackgroundColor: lineColor, pointBorderColor: 'transparent', tension: 0.3, fill: false }] },
      options: {
        ...
        scales: {
          ...
          y: { suggestedMin: yMin - yPad, suggestedMax: yMax + yPad, grid: { color: 'rgba(255,255,255,0.03)', lineWidth: 0.5 }, ticks: { color: '#7c8493', font: { family: 'Geist, sans-serif', size: 10 }, callback: (val: any) => pricingStore.formatPrice(val, price.currency) }, border: { display: false } },
        },
      },
    });
  }
...
<div class="detail-chart-container"><canvas bind:this={chartCanvas}></canvas></div>
