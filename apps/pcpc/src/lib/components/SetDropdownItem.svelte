<script lang="ts">
  /**
   * SetDropdownItem — Presentational component for a set row in the dropdown.
   *
   * Layout:
   *   [SYM]  Set Name                    [EN]
  ...
  const symbolFallback = $derived(
    set.code ? set.code.slice(0, 3).toUpperCase() : '?'
  );
  ...
  // Build subtitle parts: "CODE · 230 cards · Aug 2023"
...
<div class="set-item" class:set-item--selected={selected}>
  ...
  <div class="set-item__content">
    ...
    <div class="set-item__row1">
      ...
      <span
        class="set-item__badge"
        class:set-item__badge--en={!isJapanese}
        class:set-item__badge--jp={isJapanese}
      >
        {languageCode}
      </span>
    </div>
...
<style>
  ...
  .set-item__badge--en {
    background-color: var(--badge-en-bg);
    color: var(--badge-en-text);
  }

  .set-item__badge--jp {
    background-color: var(--badge-jp-bg);
    color: var(--badge-jp-text);
  }
