<script lang="ts">
  ...
  /** Sort order: PSA > CGC > BGS, then by grade descending */
  ...
  /** Badge colors per grading company */
  ...
  let sortedPrices = $derived(
    [...prices].sort((a, b) => {
      ...
      // Within same company, higher grade first
  ...
  /** Track which card just got copied */
  ...
  function getBadgeColors(company?: string) {
    return COMPANY_COLORS[company ?? ''] ?? DEFAULT_BADGE;
  }
</script>
    ...
    <div class="graded-grid">
      {#each sortedPrices as price, i (`graded_${i}_${price.company}_${price.grade}`)}
        ...
        <button
          ...
          onclick={() => copyGradedPrice(price, i)}
          title="Click to copy"
          type="button"
        >
          <span
