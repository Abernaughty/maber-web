...
- **printed\_total**`<integer>`: Number of cards in the set that are printed on a card. If a card is 87/160, the printed\_total is 160.
- **language**`<string>`: The language of the expansion.
- **language\_code**`<string>`: The language code of the expansion (e.g., "EN", "JA")
- **release\_date**`<string>`: The release date of the expansion, in the format YYYY/MM/DD.
- **is\_online\_only**`<boolean>`: Indicates if the set is only available online, such as Pocket expansions.
...
**expansion\_sort\_order**`<integer>`
...
| **page\_size** | The maximum number of sealed products to return per page. The highest allowable value is `100`. | `100` (max: `100`) |
...
- Combine multiple conditions:
  ...
  - Sealed products with "alakazam" in the name AND "Booster Pack" in the `type` field:
    `
    name:alakazam type:"Booster Pack"
    `
  - Sealed products with "alakazam" in the name AND either "Booster Pack" or "Booster Box" in `type`:
    `
    name:alakazam (type:"Booster Pack" OR type:"Booster Box")
    `

#### Exclude Results

- Retrieve only sealed products with `type:box` while excluding base set:
`
type:box -expansion.id:base1
`
...
name:char*
`
- Products where the name starts with "char" and ends with "der":
`
name:char*der
`
...
Fields containing numerical data (e.g., "hp", "national\_pokedex\_numbers") support range searches:
...
- Example: Request only `id` and `name` fields for all sealed products:
`
?select=id,name
`
