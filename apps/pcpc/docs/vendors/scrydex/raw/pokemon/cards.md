...
**evolves\_from**`<array of strings>`
...
- **converted\_energy\_cost**`<integer>`: The summed cost of energies for the attack.
...
**retreat\_cost**`<array of strings>`
...
**converted\_retreat\_cost**`<string>`

The converted retreat cost for a card is the count of energy types found within the retreat\_cost field.
...
**national\_pokedex\_numbers**`<array of integers>`
...
- Combine multiple conditions:
  ...
  - Cards with "charizard" in the name AND "mega" in the `subtypes` field:
    `
    name:charizard subtypes:mega
    `
  - Cards with "charizard" in the name AND either "mega" or "vmax" in `subtypes`:
    `
...
- Retrieve only cards with `subtypes:mega` while excluding water types:
`
